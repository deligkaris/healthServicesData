...
def add_orgnpinm_column_prior_year(baseDF, column="providerSepticShockVol"):
    ...
    eachOrgnpinmYear = Window.partitionBy(["ORGNPINM","THRU_DT_YEAR"]).orderBy("THRU_DT_YEAR")
    baseDF = (baseDF
              .withColumn("priorThruYear", F.lag("THRU_DT_YEAR",1).over(eachOrgnpinmYear))
              .withColumn(column+"PriorYear", F.lag(column,1).over(eachOrgnpinmYear))
