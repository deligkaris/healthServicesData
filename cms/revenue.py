...
def add_ed(revenueDF, inClaim=False):
    ...
    if (inClaim):
        eachClaim = Window.partitionBy(["DSYSRTKY","CLAIMNO","THRU_DT_DAY"])
...
def filter_claims(revenueDF, baseDF):
    ...
    revenueDF = revenueDF.join(baseDF.select(F.col("CLAIMNO"),F.col("DSYSRTKY"),F.col("THRU_DT_DAY")),
                               on=["CLAIMNO","DSYSRTKY","THRU_DT_DAY"],
                               how="left_semi")
    return revenueDF
...
def get_revenue_summary(revenueDF):
    ...
    revenueSummaryDF = revenueDF.select("DSYSRTKY", "CLAIMNO", "THRU_DT_DAY", revenueDF.colRegex("`^[a-zA-Z]+(InClaim)$`")).distinct()
